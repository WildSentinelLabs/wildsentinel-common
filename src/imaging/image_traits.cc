...
uint8_t ImageTraits::GetBitAlignment(uint8_t bit_depth) {
  if (bit_depth == 8) {
    return 1;
...
uint8_t ImageTraits::GetChromaAlignment(ChromaSubsampling subsampling) {
  switch (subsampling) {
...
bool ImageTraits::IsSigned(uint8_t bit_depth) {
