#include "streams/memory_owner_stream.h"

void MemoryOwnerStream::UpdateChunkPosition() {
  chunk_index_ = position_ / chunk_size_;
  chunk_offset_ = position_ % chunk_size_;
}

void MemoryOwnerStream::AllocateChunk() {
...
MemoryOwnerStream::MemoryOwnerStream(size_t chunk_size)
...
MemoryOwnerStream::~MemoryOwnerStream() { Dispose(); }

bool MemoryOwnerStream::CanRead() const { return is_open_; }

bool MemoryOwnerStream::CanWrite() const { return is_writable_; }

size_t MemoryOwnerStream::GetLength() const { return used_size_; }

size_t MemoryOwnerStream::GetPosition() const { return position_; }

void MemoryOwnerStream::SetPosition(size_t offset) {
...
size_t MemoryOwnerStream::Read(uint8_t buffer[], size_t offset, size_t count) {
...
bool MemoryOwnerStream::Write(const uint8_t buffer[], size_t offset,
                              size_t count) {
  if (!is_writable_) {
    return false;
  }
...
void MemoryOwnerStream::Dispose() {
  if (memory_pool_) {
    for (size_t i = 0; i < total_chunks_; ++i) {
